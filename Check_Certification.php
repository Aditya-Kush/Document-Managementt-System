...
$query = "SELECT `status`, `current_stage`, `rejection_comment`, `file_path`,  `signature_file_path` FROM `student-registeration` WHERE `urn` = ?";
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_array($result)) {
        ...
        $sig_file_path = $row['signature_file_path'];
        ...
        if ($status == "approved") {
            ...
            echo '<p> Your NDA Certificate is Approved </p> </p>';
            echo '<hr>';
            $full_file_path = "../NDA/teacher/" . $file_path; // Adjust if needed
            $signed_file_path = "../NDA/teacher/" . $sig_file_path;
            if (!empty($file_path) && file_exists($full_file_path)) {
                ...
                echo '<p class="mb-0"> You can download your certificate <a href="' . $full_file_path . '" download>using this link.</a></p>';

                // echo '<p class="mb-0"> You can download your Signed certificate <a href="' . $signed_file_path . '" target="_blank">using this link.</a></p>';
                echo '<p class="mb-0"> You can download your Signed certificate <a href="' . $signed_file_path . '" download>using this link.</a></p>';
            } else {
            ...
            echo '<p> Your NDA form is in Pending List. please wait for some time. </p> </p>';
            echo '<hr>';
